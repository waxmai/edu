<template>
  <div class="page-card">
    <div class="page-header">
      ...
      <el-button v-if="canCreateSchedule" type="primary" @click="openCreateDialog">新增排课</el-button>
    </div>
        ...
        <el-table-column label="操作" fixed="right" min-width="380">
          <template #default="scope">
            <el-space wrap>
              ...
              <el-button v-if="canReschedule" link @click="openReschedule(scope.row)">调课</el-button>
              <el-button v-if="canMakeupSchedule" link type="success" @click="openMakeupDialog(scope.row)">补课</el-button>
              <el-button v-if="canLeaveSchedule" link type="warning" @click="openLeaveDialog(scope.row)">请假</el-button>
              <el-button v-if="canCancelSchedule" link type="danger" @click="openCancelDialog(scope.row)">取消</el-button>
              <el-button v-if="canDeleteSchedule" link type="danger" @click="handleDelete(scope.row)">删除</el-button>
            </el-space>
          </template>
        </el-table-column>
      </el-table>
...
const pageKicker = computed(() => (isOrgAdmin.value ? '机构排课统筹' : isCampusAdmin.value ? '校区排课执行' : '排课调度'))
const pageTitle = computed(() => (isOrgAdmin.value ? '机构排课管理' : isCampusAdmin.value ? '校区排课管理' : '排课管理'))
const pageDesc = computed(() => (isOrgAdmin.value ? '统筹本机构排课安排、资源协调与调补课衔接。' : isCampusAdmin.value ? '集中维护本校区排课安排、状态流转与现场调补课。' : '集中维护排课安排、状态流转与调补课衔接。'))
const canCreateSchedule = computed(() => can({ permission: 'schedule:create', menuPermission: 'menu:schedule:view' }))
const canMakeupSchedule = computed(() => can({ permission: 'schedule:makeup', menuPermission: 'menu:schedule:view' }))
const canCancelSchedule = computed(() => can({ permission: 'schedule:cancel', menuPermission: 'menu:schedule:view' }))
const canDeleteSchedule = computed(() => can({ permission: 'schedule:delete', menuPermission: 'menu:schedule:view' }))
const canReschedule = computed(() => can({ permission: 'schedule:reschedule', menuPermission: 'menu:schedule:view' }))
const canLeaveSchedule = computed(() => can({ permission: 'schedule:leave', menuPermission: 'menu:schedule:view' }))
const canManageSchedule = computed(() => canCreateSchedule.value || canMakeupSchedule.value || canCancelSchedule.value || canDeleteSchedule.value)
...
function openMakeupDialog(row: ScheduleListItem) {
  if (!canMakeupSchedule.value) {
    message.warning('当前角色不能新增补课排课')
    return
  }
...
function openLeaveDialog(row: ScheduleListItem) {
  if (!canLeaveSchedule.value) {
    message.warning('当前角色不能发起请假')
    return
  }
...
function openCancelDialog(row: ScheduleListItem) {
  if (!canCancelSchedule.value) {
    message.warning('当前角色不能取消排课')
    return
  }
