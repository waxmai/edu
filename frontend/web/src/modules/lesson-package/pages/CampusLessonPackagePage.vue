<template>
  <div class="page-card">
    <div class="page-header">
      <div>
        <div class="hero-kicker">校区课时经营</div>
        <h2>校区课时包管理</h2>
        <p>维护本校区学员课时包，聚焦剩余课时、到期与续费。</p>
      </div>
    ...
    <el-alert
      title="当前页面聚焦本校区课时包维护，便于处理续费、缴费和到期跟进。"
      type="success"
      :closable="false"
    />
...
function goSignupPayment(row: LessonPackageItem) {
  router.push(`/campus/payments?studentId=${row.studentId}&lessonPackageId=${row.id}&paymentType=signup`)
}

function goRenewPayment(row: LessonPackageItem) {
  router.push(`/campus/payments?studentId=${row.studentId}&lessonPackageId=${row.id}&paymentType=renewal`)
}
</script>
