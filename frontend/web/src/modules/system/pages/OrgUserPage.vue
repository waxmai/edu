<template>
  <div class="page-card">
    <div class="page-header">
      <div>
        <div class="hero-kicker">机构账号治理</div>
        <h2>机构用户管理</h2>
        <p>统一维护本机构账号、角色、状态与登录安全治理。</p>
      </div>
      <div class="actions">
    ...
    <el-alert
      title="当前页面面向机构管理员，可维护本机构账号、校区管理员和教师账号。"
      type="success"
      :closable="false"
    />
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑用户' : '新增用户'" width="520px">
      <el-form label-position="top" :model="form">
        ...
        <el-form-item label="机构">
          <el-input model-value="当前机构" disabled />
        </el-form-item>
        <el-form-item label="校区">
          <el-select v-model="form.campusId" clearable filterable style="width: 100%">
            <el-option v-for="item in campusOptions" :key="item.id" :label="item.name" :value="item.id" />
          </el-select>
        </el-form-item>
...
function formatScope(user: SystemUserItem) {
  if (user.roleCode === 'org_admin') return '机构级（当前机构）'
...
function formatCampusName(campusId?: number) {
  return campusOptions.value.find((item) => item.id === campusId)?.name || '-'
}
...
async function loadUsers() {
  loading.value = true
  try {
    const response = await fetchSystemUsers(cleanQueryParams({ ...query }))
