  ...
  <div class="page-card">
    <div class="page-header">
      ...
      <div class="actions">
        ...
        <el-button v-if="isPlatformAdmin" type="primary" @click="openOrgCreate">新增机构</el-button>
        <el-button type="success" @click="openCampusCreate">新增校区</el-button>
      </div>
    </div>
    ...
    <div class="platform-grid">
      <el-card v-if="isPlatformAdmin" shadow="never" class="section-card data-table-card" v-loading="loadingOrganizations">
        <template #header>
        ...
        <el-table :data="organizations" border>
          ...
          <el-table-column label="操作" width="220" fixed="right">
            <template #default="scope">
              <el-space wrap>
                <el-button v-if="isPlatformAdmin" link type="primary" @click="openOrgEdit(scope.row)">编辑</el-button>
                <el-button v-if="isPlatformAdmin" link @click="enterOrgView(scope.row)">进入机构视角</el-button>
              </el-space>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-card shadow="never" class="section-card data-table-card" v-loading="loadingCampuses">
        ...
        <el-table :data="campuses" border>
          ...
          <el-table-column label="操作" width="220" fixed="right">
            <template #default="scope">
              <el-space wrap>
                <el-button link type="primary" @click="openCampusEdit(scope.row)">编辑</el-button>
                <el-button v-if="isPlatformAdmin" link @click="enterCampusView(scope.row)">进入校区视角</el-button>
              </el-space>
            </template>
          </el-table-column>
        </el-table>
...
import {
  createPlatformCampus,
  createPlatformOrganization,
  fetchPlatformCampuses,
  fetchPlatformOrganizations,
  updatePlatformCampus,
  updatePlatformOrganization,
  type PlatformCampusItem,
  type PlatformCampusPayload,
  type PlatformOrganizationItem,
  type PlatformOrganizationPayload,
} from '@/api/platform'
...
const pageTitle = computed(() => (isPlatformAdmin.value ? '机构与校区' : '校区管理'))
const pageDescription = computed(() => (isPlatformAdmin.value ? '统一维护机构开通状态、校区结构与基础能力范围。' : '维护本机构校区结构与可用状态，支撑机构日常运营。'))
const pageAlert = computed(() => (isPlatformAdmin.value ? '订阅已过期或停用的机构将被后端禁止关键业务写操作。平台管理员可在此维护机构和校区基础信息，并可进入机构/校区视角排障。' : '机构管理员可在本机构范围内维护校区基础信息，支持新增和编辑校区。'))
...
async function loadData() {
  ...
  try {
    ...
    message.error(extractErrorMessage(error, '加载机构/校区数据失败，请稍后重试'))
  } finally {
...
function resetCampusForm() {
  campusForm.organizationId = organizations.value[0]?.id || 1
