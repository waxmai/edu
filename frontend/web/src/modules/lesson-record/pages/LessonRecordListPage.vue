<template>
  <div class="page-card">
    <div class="page-header">
      <div>
        <div class="hero-kicker">课堂执行</div>
        <h2>上课记录</h2>
        <p>统一沉淀课堂内容、作业反馈与课时扣减结果。</p>
      </div>
      <el-button v-if="canCreateLessonRecord" type="primary" @click="openCreateDialog">新增上课记录</el-button>
    </div>

    <el-alert
      type="success"
      :closable="false"
      title="可按学员、教师和出勤状态筛选上课记录，并维护课堂内容、作业与反馈。"
    />
      ...
      <el-table :data="rows" border>
        ...
        <el-table-column label="操作" fixed="right" min-width="160">
          <template #default="scope">
            <el-space>
              <el-button v-if="canEditLessonRecord" link type="primary" @click="openEditDialog(scope.row)">编辑</el-button>
              <el-button v-if="canDeleteLessonRecord" link type="danger" @click="handleDelete(scope.row)">删除</el-button>
            </el-space>
          </template>
        </el-table-column>
      </el-table>
...
const canCreateLessonRecord = computed(() => can({ permission: 'lesson-record:create', menuPermission: 'menu:lesson-record:view' }))
const canEditLessonRecord = computed(() => can({ permission: 'lesson-record:update', menuPermission: 'menu:lesson-record:view' }))
const canDeleteLessonRecord = computed(() => can({ permission: 'lesson-record:delete', menuPermission: 'menu:lesson-record:view' }))
...
function openCreateDialog() {
  if (!canCreateLessonRecord.value) {
    message.warning('没有新增上课记录权限')
    return
  }
...
function openEditDialog(row: LessonRecordItem) {
  if (!canEditLessonRecord.value) {
    message.warning('没有编辑上课记录权限')
    return
  }
...
async function handleSubmit() {
  if (editingId.value && !canEditLessonRecord.value) {
    message.warning('没有编辑上课记录权限')
    return
  }
  if (!editingId.value && !canCreateLessonRecord.value) {
    message.warning('没有新增上课记录权限')
    return
  }
...
async function handleDelete(row: LessonRecordItem) {
  if (!canDeleteLessonRecord.value) {
    message.warning('没有删除上课记录权限')
    return
  }
  try {
